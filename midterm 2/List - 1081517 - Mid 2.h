...
public:
		...
		: myData()
	{
		...
		for (size_type i = 0; i < right.size(); i++, it2++)
			insert(it, *it2);  //modified
	}
	...
	void insert(const_iterator where, const value_type& val) // insert val at where
	{
		...
		new_node->prev = where.ptr->prev;
		where.ptr->prev->next = new_node;
		new_node->next = where.ptr;
		where.ptr->prev = new_node;
