...
int main()
{
   ...
   HugeInteger n1[ numTestCases ];  //n1是一個HugeInteger陣列，裡面每個元素都是HugeInteger
...
void input( HugeInteger n1[], HugeInteger n2[] )
{
   ...
   for( int i = 0; i < numTestCases; i++ )
   {
      ...
      //txt檔靠左對齊，所以要用到convert(但是靠右對齊又很麻煩)，convert的用意是要轉個方向，對齊位數
      n1[ i ].convert( vector1 );  //converts its type from "vector" to "integer"
