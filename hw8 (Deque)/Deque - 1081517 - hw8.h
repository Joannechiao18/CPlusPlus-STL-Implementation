...
public:
   ...
   const MyDeque *myCont; // keep a pointer to deque (指向myData,myDeque就是DequeVal)
...
public:
   ...
   {
      MyBase::operator++();  //因為繼承，用myBase::取用operator++
      return *this;
   }
...
public:
   ...
   iterator begin()
   {
      ...
      //進入DequeIterator constructor建立物件，因為繼承所以進入DequeConstIterator初始化myBase建立另一個物件
   }
   ...
   void push_front( const Ty &val )
   {
       if (myData.mySize == 0)  
       {
           if (myData.mapSize == 0)  //mySize=0有兩種情況:一個是一開始連map都還沒創建，另一個是空間已經存在，但因為元素pop掉，所以mySize變成0
               myData.mapSize = 8;
           ...
           myData.myOff = 4 * myData.mapSize - 1;  //重新調整myOff的位置(一開始出示化為0,所以push_back不用調整)
       }
       else
       {
           size_type newfront = (myData.myOff - 1) % (4 * myData.mapSize);  //要取餘數因為可能有0-1=-1的情況
   ...
   void pop_front()
   {
      if( --myData.mySize == 0 )  //mySize=1, 唯一的元素在位置31
         myData.myOff = 0;
      else
         ++myData.myOff;
   }
   ...
   void push_back( const Ty &val )
   {
      ...
      else
      {
         ...
         else  //假設size=10，myOff在28，mapSize=32，代表0開始後還有7個元素，但是在pop_front的時候會myOff++
...
//      return ( ( myData.off / 4 ) & ( myData.mapSize - 1 ) );
   ...
   void doubleMapSize()
   {
      if( myData.mapSize > 0 )
      {
          ...
          for (; i < (myData.myOff + myData.mySize) / 4; i++)  //沒用到還是nullPtr
              newMap[i] = new Ty[4]();  
          ...
          for (; i < myData.myOff + myData.mySize; i++, j++)
          {
              /*if (j == 4 * myData.mapSize)
                  j = 0;*/
              newMap[i / 4][i % 4] = myData.map[j % (4 * myData.mapSize) / 4][j % 4];

              //newMap[i% (4 * myData.mapSize) / 4][i % 4] = myData.map[j % (4 * myData.mapSize) / 4][j % 4]會爆掉
              //因為newMap的位置應該由i從myOff一直加到myData.myOff + myData.mySize決定，j才要判斷從31到0的情況
          }

          //for(size_type i=myData.mySize;i>=0;i--)會爆掉，因為根本沒有由myData.mySize這的位置指出的動態陣列
          for (size_type i = 0; i < myData.mapSize; i++)
              if (myData.map[i] != nullptr)
                  delete[] myData.map[i];
          delete[] myData.map;
