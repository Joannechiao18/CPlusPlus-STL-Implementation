...
public:
   ...
   void clear() // erase all
   {
       if (myData.mySize != 0) // the list is not empty
       {
           ...
           {
               myData.myHead->next = myData.myHead->next->next;  //先移動myData.myHead->next的位置，下面才可以delete
               //myData.myHead->next->prev = myData.myHead;  //這樣會刪掉myHead
