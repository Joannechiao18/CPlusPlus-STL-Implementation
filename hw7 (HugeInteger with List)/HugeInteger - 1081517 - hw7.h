...
#include "Vector - 1081517 - hw7.h" // include definition of class template vector
#include "List - 1081517 - hw7.h"   // include definition of class template list
...
{ 
    ...
    typename T::reverse_iterator it1 = integer.rbegin();  //相當於it1指向end的位置，但是取值的時候是取到前一個
    typename T::reverse_iterator it2 = right.integer.rbegin();

    for (; it2 != right.integer.rend(); it1++, it2++)  //begin的位置的值也會被比較到，因為在it2=right.integer.end()+1的時候就比較到了
    {
        if (*it1 < *it2)
            return true;
        else if (*it1 > *it2)
    ...
    it2--;
    for(;it2>=right.integer.begin();it1--,it2--)*/  //因為iterator沒有>=的函數，盡量用==和!=
...
HugeInteger< T > HugeInteger< T >::operator-( HugeInteger< T > &op2 )
{
   ...
   //typename T::reverse_iterator it5 = difference.integer.rbegin();  //指在end的位置，但下面從end()-1的地方判斷是否為零
   //while (*it5 == 0 && difference.integer.size() > 1)
   //    difference.integer.pop_back();
   typename T::iterator it5 = difference.integer.end();
   it5--;
   while (*it5 == 0 && difference.integer.size() > 1)
   {
       it5--;
...
HugeInteger< T > HugeInteger< T >::operator*( HugeInteger< T > &op2 )
{
   ...
   {
       it4 = it3;
       for (it1 = integer.begin(); it1 != integer.end(); it1++, it4++)
       {
           int n = (*it4) + (*it2) * (*it1);
           if (n > 9)
           {
   ...
   typename T::iterator it5 = product.integer.end();
   it5--;
   while (*it5 == 0 && product.integer.size() > 1)
   {
       it5--;
...
{
   typename T::iterator it = integer.begin();  //VectorIterator< VectorVal< int > > it = integer.begin();
