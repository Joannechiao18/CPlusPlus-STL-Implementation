...
void testCopyConstructor()
{
   ...
   for( unsigned int i = 0; i < number; i++ )
   {
      ...
      for( unsigned int j = 0; j < length; j++ )
      {
         ...
         /*cout << string1.begin()[j] << ' ';*/
         string2.push_back( value );
         /*cout << string2.begin()[j] << endl;*/
      }
...
void testResize1()
{
   ...
   for( unsigned int i = 0; i < number; i++ )
   {
      ...
      if( equal( str1, str2 ) )  //string1沒有被截斷，只是mySize有重新設定，然後測equal只會比兩個string size有沒有一樣
         numErrors--;
   }
...
void testResize2()
{
   ...
   for( unsigned int i = 0; i < number; i++ )
   {
      ...
      if( str.size() > oldCapacity )  //檢查如果容量不夠要new一個新陣列(在新的位置)
      {
