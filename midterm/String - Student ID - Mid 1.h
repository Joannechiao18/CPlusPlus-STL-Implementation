...
public:
   ...
   iterator insert( const_iterator where, const char ch )
   {
      ...
      {
          int newSize = size() + 1, i = 0, n = where - begin();
          if (newSize <= capacity())
          {
              for (i = size(); i > n; i--)  //notice that i cannot be typed as int i, or else it will be initialized again
                  myData.myPtr()[i] = myData.myPtr()[i - 1];
          ...
          {
              size_type newmyRes = myData.myRes * 1.5;
              if (newmyRes < (newSize / 16) * 16 + 15)
                  newmyRes = (newSize / 16) * 16 + 15;

              pointer v1 = new value_type[newmyRes + 1]();
              for (i = 0; i < n; i++)
                  v1[i] = myData.myPtr()[i];
              ...
              for (; i < newSize; i++)  //if i<mySize the last element will not be copied
                  v1[i] = myData.myPtr()[i - 1];
