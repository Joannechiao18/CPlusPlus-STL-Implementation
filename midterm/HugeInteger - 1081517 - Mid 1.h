...
#include "Vector-1081517-Mid - 1.h" // include definition of class vector 
...
HugeInteger< T > HugeInteger< T >::square()
{
   ...
   typename T::iterator it2 = integer.begin();  //added
   typename T::iterator it3 = square.integer.begin();
   int i;
   for (; it1 != integer.end(); it1++, it3++)
       for (i = 0, it2 = integer.begin(); it2 != integer.end(); it2++, i++)  //modified
           *(it3 + i) += (*it1) * (*it2);

   for (it3 = square.integer.begin(); it3 != square.integer.end(); it3++)
   {
       if (*it3 > 9)
       {
           *(it3 + 1) += *(it3) / 10;
   ...
   it3 = square.integer.end() - 1;
   while (*it3 == 0 && square.integer.size() > 1)
   {
       it3--;
...
HugeInteger< T > HugeInteger< T >::squareRoot()
{
   ...
   {	   
	   sqrt.integer.begin()[k]++;

       while (!((*this) < sqrt.square()))
	   {
		   sqrt.integer.begin()[k]++;
	   }
	   sqrt.integer.begin()[k]--;
