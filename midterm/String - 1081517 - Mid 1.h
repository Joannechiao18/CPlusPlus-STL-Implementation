...
public:
   ...
   iterator insert( const_iterator where, const char ch )
   {
      ...
      {
          int newSize = size() + 1, i = 0, n = where - begin();  //modified
          if (newSize <= capacity())
          {
              for (i = size(); i > n; i--)  //added
                  myData.myPtr()[i] = myData.myPtr()[i - 1];

              myData.myPtr()[i] = ch;  //modified
          ...
          else if (newSize > capacity())
          {
              ...
              pointer v1 = new char[newmyRes + 1]();  //modified
              for (i = 0; i < n; i++)  //added
                  v1[i] = myData.myPtr()[i];
              v1[i] = ch;  //added
              for (i += 1; i < newSize; i++)  //added
                  v1[i] = myData.myPtr()[i - 1];

              if (myData.myRes > 15)
                  delete[] myData.bx.ptr;

              myData.bx.ptr = v1;  //added
              myData.mySize = newSize;
              myData.myRes = newmyRes;
          }
		  
		 // if(where=this->end())  //deleted
			//myData.myPtr()[size()] = ch;  //deleted
		 // else  //deleted
			//this->begin()[*where] = ch;  //deleted
      }
      else
         return nullptr;
   }
