...
public:
   ...
   deque& operator=( const deque &right )
   {
      if( &right != this ) // avoid self-assignment
      {
          ...
          size_type i = myData.myOff, j = right.myData.myOff;
          for (; i < myData.myOff + myData.mySize; i++, j++)
              myData.map[i % (4 * myData.mapSize) / 4][i % 4] = right.myData.map[j % (4 * myData.mapSize) / 4][j % 4];

          /*(2) clear();
          typename deque<Ty>::const_iterator it = right.begin();
          for (; it != right.end(); it++)
              push_back(*it);*/

          //resize(right.myData.mySize);
         /*(3) for (int i = 0; i < right.size(); i++)
              this->begin()[i] = right.begin()[i];*/

          /*(4) typename deque<Ty>::iterator it1 = this->begin();
   ...
   void push_back( const value_type &val )
   {
       ...
       else
       {
           ...
           myData.map[newBack / 4][newBack%(4*myData.mapSize) % 4] = val;
       }
       ++myData.mySize;
   }
...
bool operator==( const deque< Ty > &left, const deque< Ty > &right )
{
    ...
    typename deque<Ty>::const_iterator it2 = right.begin();
    int i, j;
    for (i = j = 0; i < left.size(), j < right.size(); i++, j++, it1++, it2++)
    {
        if (*it1 != *it2)
            return false;
    }
